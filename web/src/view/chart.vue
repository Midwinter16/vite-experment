...
const initSeries = (data: []) => {
  ...
  if (!temp.tooltips.includes('x轴辅助线')) Reflect.deleteProperty(tooltip, 'axisPointer')
  return {
    title: {
      text: '预览图',
      x: 'center',
...
const initCharts = (options: any) => {
  const mycharts = echarts.init(document.querySelector('#charts-main') as HTMLDivElement)
  console.log(options)
